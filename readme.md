...
```
ensure Prime-Parking
```
### Important Note:
All garage data of pre-existing homes will not be compatible with this garage, you need to delete those garages and create new ones. When creating a new garage, you need to be in a vehicle.
...
RegisterNUICallback('track-vehicle', function(data, cb)
    local veh = data.veh
    TriggerEvent('Prime-Parking:client:trackVehicle', veh.plate)
end)
```
...
RegisterNetEvent('qb-houses:server:addGarage', function(house, coords)
    ...
    TriggerClientEvent("Prime-Parking:client:updateGarage", -1) 	-- Update Garages
...
- Event:
```
'Prime-Parking:client:openGarage'
```
- Enable Menu(If your radial menu can enable/hide button):
```
local isingarage, canStoreVehicle = exports["Prime-Parking"]:IsInGarage()
```
...
- Event:
```
'Prime-Parking:client:storeVehicle'
```
- Enable Menu(If your radial menu can enable/hide button):
```
local isingarage, canStoreVehicle = exports["Prime-Parking"]:IsInGarage()
...
- Event:
```
'Prime-Parking:client:openJobVehList'
```
- Enable Menu(If your radial menu can enable/hide button):
```
isInJobGarage, lastJobVehicle = exports["Prime-Parking"]:isInJobStation('your job')
```
...
- Event:
```
'Prime-Parking:client:HideJobVeh'
```
- Enable Menu(If your radial menu can enable/hide button):
```
isInJobGarage, lastJobVehicle = exports["Prime-Parking"]:isInJobStation('your job')
...
I am not the original creator of this script. However this is the version before real time parking. Also this is before he added the requirement for this other scripts.

Link: https://github.com/hoangducdt/MojiaGarages
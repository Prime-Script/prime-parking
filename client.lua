...
RegisterNetEvent('QBCore:Client:OnPlayerLoaded', function() -- Event when player has successfully loaded
    TriggerEvent('Prime-Parking:client:DestroyingZone') -- Destroying all zone
	Wait(100)
	PlayerData = QBCore.Functions.GetPlayerData() -- Reload player information
	Wait(100)
	TriggerServerEvent('Prime-Parking:server:updateHouseKeys') 	-- Reload house key information	
	Wait(100)
	TriggerServerEvent('Prime-Parking:server:UpdateGaragesZone') -- Reload garage information
	Wait(100)
...
RegisterNetEvent('Prime-Parking:client:UpdateGaragesZone', function(garageConfig) -- Update Garages Zone
...
RegisterNetEvent('Prime-Parking:client:DestroyingZone', function() -- Destroying all zone
...
RegisterNetEvent('Prime-Parking:client:updateGarage', function() -- Update Garages
	TriggerEvent('Prime-Parking:client:DestroyingZone') -- Destroying all zone
	Wait(100)
	PlayerData = QBCore.Functions.GetPlayerData() -- Reload player information
	Wait(100)
...
RegisterNetEvent('Prime-Parking:client:updateHouseKeys', function(keylist) --Update House Keys
	if keylist then
		HouseKeys = keylist
	end
end)

RegisterNetEvent('Prime-Parking:client:openGarage', function() -- Garages Menu
    if inGarageStation and currentgarage ~= nil then
		QBCore.Functions.TriggerCallback('Prime-Parking:server:GetUserVehicles', function(result)
			if result then
						...
						if v.state == 1 then
							if v.garage == currentgarage then
								...
								MenuGaraOptions[#MenuGaraOptions + 1] = {
									...
									params = {
										event = 'Prime-Parking:client:TakeOutVehicle',
										args = v
									}
								}
							end
...
RegisterNetEvent('Prime-Parking:client:TakeOutVehicle', function(vehicle) -- Option to take the vehicle out
    if inGarageStation and currentgarage ~= nil and nearspawnpoint ~= nil then
		if vehicle.state == 0 and vehicle.depotprice > 0 then
			TriggerServerEvent('Prime-Parking:server:PayDepotPrice', vehicle)
			Wait(1000)
		else
			TriggerEvent('Prime-Parking:client:doTakeOutVehicle', vehicle)
		end
	end
end)

RegisterNetEvent('Prime-Parking:client:doTakeOutVehicle', function(vehicle) -- Take the vehicle out
    if inGarageStation and currentgarage ~= nil and nearspawnpoint ~= nil then
		...
		if not IsSpawnPointClear(vector3(Garages[currentgarage].spawnPoint[lastnearspawnpoint].x, Garages[currentgarage].spawnPoint[lastnearspawnpoint].y, Garages[currentgarage].spawnPoint[lastnearspawnpoint].z), 2.5) then
			...
			QBCore.Functions.SpawnVehicle(vehicle.vehicle, function(veh)
				QBCore.Functions.TriggerCallback('Prime-Parking:server:GetVehicleProperties', function(properties)
					...
					if vehicle.plate ~= nil then
						...
						TriggerServerEvent('Prime-Parking:server:UpdateOutsideVehicles', OutsideVehicles)
					end
					...
					TriggerServerEvent('Prime-Parking:server:updateVehicleState', 0, vehicle.plate, vehicle.garage)
...
RegisterNetEvent('Prime-Parking:client:storeVehicle', function() -- Store Vehicle
    if inGarageStation and currentgarage ~= nil then
		...
		if Garages[lastcurrentgarage].garastate == 1 then
			...
			if exports['qb-vehiclekeys']:HasKey(plate) then
				if curVeh and #(pos - vehpos) < 7.5 then
					QBCore.Functions.TriggerCallback('Prime-Parking:server:checkVehicleOwner', function(owned)
						if owned then					
							...
							TriggerServerEvent('Prime-Parking:server:updateVehicleStatus', totalFuel, engineDamage, bodyDamage, plate, lastcurrentgarage)
							TriggerServerEvent('Prime-Parking:server:updateVehicleState', 1, plate, lastcurrentgarage)
							if plate ~= nil then
...
RegisterNetEvent('Prime-Parking:client:openJobVehList', function() --Job Vehicles Menu
	PlayerData = QBCore.Functions.GetPlayerData()
	if lastjobveh ~= nil then
		...
		for k, v in pairs(JobVeh[PlayerData.job.name][currentgarage].vehicle[PlayerData.job.grade.level]) do
			...
			vehicleMenu[#vehicleMenu + 1] = {
				...
				params = {
					event = 'Prime-Parking:client:SpawnJobVeh',
					args = {
						model = k,
						plate = plate,
						livery = v.livery,
...
RegisterNetEvent('Prime-Parking:client:SpawnJobVeh', function(data) -- Take vehicle for job
	local pos = nil
	local header = nil
	local lastnearspawnpoint = nearspawnpoint
...
RegisterNetEvent('Prime-Parking:client:HideJobVeh', function() -- Hide vehicle for job
	local ped = PlayerPedId()
	local pos = GetEntityCoords(ped)
	local curVeh = QBCore.Functions.GetClosestVehicle(pos)
	if IsPedInAnyVehicle(ped) then
	...
	local plate = QBCore.Functions.GetPlate(curVeh)
	if exports['qb-vehiclekeys']:HasKey(plate) and curVeh == lastjobveh then
		if IsPedInAnyVehicle(ped) then
			CheckPlayers(curVeh)
		else
			QBCore.Functions.DeleteVehicle(curVeh)
...
CreateThread(function() -- Update for qb-radialmenu
	while true do		
		if inGarageStation and currentgarage ~= nil then
			TriggerEvent('Prime-Parking:client:updateRadialmenu')
		else
			TriggerEvent('Prime-Parking:client:updateRadialmenu')
		end
		Wait(1000)
...
RegisterNetEvent('Prime-Parking:client:updateRadialmenu', function()
